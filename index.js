...
if (!HOME) {
  console.log(
    "Error: HOME is not set. Please set a HOME environment variable.",
  );
}
...
// Has the multi-select been initialised?
...
const suggest = (input, choices) => {
  // Hack to initialise the filter with any additional arguments.
  if (!input && additionalArgs && !initialized) {
...
const response = await prompts({
  ...
  suggest,
  onRender() {
    if (!this.input && additionalArgs && !initialized) {
      this.input = additionalArgs;
      this.cursor = additionalArgs.length;
    }
    initialized = true;
  },
});
